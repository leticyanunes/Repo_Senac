...
CREATE TABLE vendas(data_venda DATE, produto VARCHAR(100), categoria VARCHAR(100), 
valor_unitario DECIMAL(10,2), quantidade INT, cliente VARCHAR(100), 
cidade VARCHAR(100), estado VARCHAR(2) );

SELECT *FROM vendas;

INSERT INTO vendas(data_venda, produto, categoria, valor_unitario, quantidade, cliente,
cidade, estado) VALUES('2025-09-15', 'Notebook Dell Inspiron', 'Informática', '3500.00', 
1, 'João Silva', 'Rio de Janeiro', 'RJ'); 
...
SELECT produto, categoria, valor_unitario FROM vendas WHERE categoria = 'Informática';

SELECT cliente, produto, quantidade FROM vendas WHERE valor_unitario > 1000;

SELECT produto, valor_unitario FROM vendas ORDER BY valor_unitario DESC;

SELECT SUM(quantidade * valor_unitario) AS Valor_Vendido FROM vendas;

SELECT estado, COUNT(*) AS quantidade_de_venda FROM vendas GROUP BY estado;

SELECT estado, SUM(quantidade * valor_unitario) AS Valor_Vendido FROM 
vendas GROUP BY estado;

SELECT cliente, SUM(valor_unitario * quantidade) AS total_cliente FROM vendas
WHERE categoria = 'Informática' GROUP BY cliente HAVING total_cliente > 
5000;

SELECT categoria, AVG(valor_unitario * quantidade) AS media_vendida FROM vendas
 GROUP BY categoria;
 
 SELECT produto, valor_unitario FROM vendas;
 
 SELECT estado, COUNT(*) AS Vendas_SP FROM vendas GROUP BY estado 
 HAVING estado = 'SP';
 
 SELECT SUM(quantidade * valor_unitario) AS Valor_vendido FROM vendas;
 
 SELECT categoria, SUM(quantidade * valor_unitario) AS Valor_Vendido_Por_Categoria
 FROM vendas GROUP BY categoria;
 
 SELECT cliente, SUM(valor_unitario * quantidade) AS Valor_Acima_de_3mil FROM vendas
 GROUP BY cliente HAVING Valor_Acima_de_3mil > 3000;
 
 SELECT cidade, SUM(valor_unitario * quantidade) AS Vendas_Curitiba FROM vendas 
 GROUP BY cidade HAVING cidade = 'Curitiba';
 
 SELECT cliente, SUM(valor_unitario * quantidade) AS Vendas_Acima_de_4mil FROM vendas
 GROUP BY cliente HAVING Vendas_Acima_de_4mil > 4000;
 
 SELECT cliente, SUM(valor_unitario * quantidade) AS Vendas_Acima_de_5mil FROM vendas
 GROUP BY cliente HAVING Vendas_Acima_de_5mil > 5000;
 
 SELECT cliente, COUNT(*) AS Mais_200 FROM vendas Where valor_unitario > 200 
 GROUP BY cliente;
 
 SELECT estado, SUM(quantidade * valor_unitario) AS total_vendido FROM vendas Where 
 cidade IN ('Curitiba', 'Porto Alegre', 'Belo Horizonte') GROUP BY estado 
 HAVING total_vendido > 5000;
 
 SELECT cliente, ROUND(AVG(quantidade * valor_unitario),2) AS media_por_cliente FROM vendas
 where categoria = 'Informática' GROUP BY cliente HAVING media_por_cliente > 100;
